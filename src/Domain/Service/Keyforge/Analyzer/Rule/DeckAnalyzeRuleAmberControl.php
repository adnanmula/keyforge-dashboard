...
final class DeckAnalyzeRuleAmberControl implements DeckAnalyzeRule
{
    ...
    public function execute(KeyforgeDeck $deck): ?array
    {
        ...
        $r[] = $this->ruleHasCards($deck, 'Recursión de Banda', 'Hit and Run', 'Chain Gang');
        $r[] = $this->ruleHasCards($deck, 'Recursión de Ronnie', 'Hit and Run', 'Ronnie Wristclocks');
        $r[] = $this->ruleHasCards($deck, 'Recursión de Bo Nithing', 'Hit and Run', 'Bo Nithing');
        $r[] = $this->ruleHasCards($deck, 'Captura y obtiene ámbar', 'City-State Interest', 'Bury Riches');
        $r[] = $this->ruleHasCards($deck, 'Recursión de Inforno', 'Infurnace', 'Hysteria');
